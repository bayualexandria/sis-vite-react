...
function UpdateKelasById({ id }) {
  ...
  const getDataKelas = async () => {
    try {
      let response = await api.get(`kelas/data-kelas`).then((res) => res.data);
  ...
  const dataKelasHistory = async () => {
    try {
      let response = await api.get(`kelas/${id}`).then((res) => res.data);
              ...
              >
                ...
                <div className="flex flex-col gap-y-2">
                  ...
                  >
                    ...
                    {guru.map((data) => {
                      if (getKelas.guru_wali_id === data.id) {
                        return (
                          <option
                            value={getKelas.guru_wali_id}
                            key={getKelas.guru_wali_id}
                            selected
                          >
                            {data.name}
                          </option>
                ...
                <div className="flex flex-col gap-y-2">
                  ...
                  >
                    ...
                    {/* {semester.map((data) => {
                      if (getKelas.semester_id === data.id) {
                        return (
                          <option
                            value={getKelas.semester_id}
                            key={getKelas.semester_id}
                            selected
                          >
                            Semester{" "}
                            {data.semester == "II" ? "Ganap" : "Ganjil"}
                            {"-"}
